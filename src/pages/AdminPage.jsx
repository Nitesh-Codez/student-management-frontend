...
export default function AdminPage() {
  ...
  useEffect(() => {
    axios
      .get(`${API_URL}/api/new-marks/classes`)
      .then((res) => {
        if (res.data.success) {
          ...
          <div key={task} style={{ marginTop: 30 }}>
            ...
            {subs.length === 0 ? (
              ...
              <table style={tableStyle}>
                <thead>
                  <tr>
                    <th style={thHeaderStyle}>Student</th>
                    <th style={thHeaderStyle}>Subject</th>
                    <th style={thHeaderStyle}>Uploaded Time</th>
                    <th style={thHeaderStyle}>Deadline</th>
                      ...
                      <tr key={s.id}>
                        ...
                        <td style={thTdStyle}>
                          {[1, 2, 3, 4, 5].map((i) => (
                            <span
                              key={i}
                              onClick={() => handleRating(s.id, i)}
                              style={{
                                cursor: "pointer",
                                color: s.rating >= i ? "#ffc107" : "#ccc",
                                fontSize: "18px",
                                marginRight: 2,
                              }}
                            >
