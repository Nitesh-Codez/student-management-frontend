...
  
  // Load dynamically from localStorage on boot up mapped by student ID
  const [activeSubjects, setActiveSubjects] = useState(() => {
    if (!studentId) return [];
    const savedActiveStore = JSON.parse(localStorage.getItem("userActiveSubjects")) || {};
    return savedActiveStore[studentId] || [];
  });
...
  /**
   * BACKGROUND SYNC: NEW MARKS DETECTOR (Length & ID Based Protection)
   */
...
          
          // फिक्स 1: अगर डेटाबेस के टोटल रिकॉर्ड लोकल रिकॉर्ड से कम या बराबर हैं, तो पॉपअप को रोक दो
          if (currentLocalMarks.length >= fetchedData.length) {
            return; 
          }

          const currentIds = currentLocalMarks.map((m) => m.id);
          
          // केवल वही अनसीन रिकॉर्ड फ़िल्टर करें जो लोकल एरे में नहीं हैं
          const newEntries = fetchedData.filter((m) => !currentIds.includes(m.id));

          if (newEntries.length > 0) {
            // अगर लोकल मार्क्स का डेटा पूरी तरह खाली है (जैसे पहली बार लॉगिन करने पर), तो बिना पॉपअप के सीधे सेट कर दो
...
            // अगर पहले से कुछ मार्क्स थे और अब एरे की लेंथ बढ़ी है, तब असली पॉपअप दिखाओ
...
          setMarks(fetchedData);
          
          // अपडेटेड मार्क्स तुरंत लोकल स्टोरेज में डाल दो ताकि लेंथ मैच हो जाए
          const savedMarks = JSON.parse(localStorage.getItem("userMarks")) || {};
          savedMarks[userRef.current.id] = fetchedData;
          localStorage.setItem("userMarks", JSON.stringify(savedMarks));

          // रीसेंट अनचेक्ड अलर्ट्स क्लियर करें
...
  // पॉपअप क्लोज करने पर मार्क्स को लोकल स्टोरेज में सिंक करने का हैंडलर
...
    localStorage.setItem("latestCheckHistory", JSON.stringify(historyStore));
    
    // वर्तमान मार्क्स स्टेट को सेव कर दो ताकि अगली बार लॉगिन पर यह फेच्ड डेटा की लंबाई से बराबर रहे
    const savedMarks = JSON.parse(localStorage.getItem("userMarks")) || {};
    savedMarks[userRef.current.id] = marks;
    localStorage.setItem("userMarks", JSON.stringify(savedMarks));
  };
...
            
            const savedActiveStore = JSON.parse(localStorage.getItem("userActiveSubjects")) || {};
            savedActiveStore[studentId] = updated;
            localStorage.setItem("userActiveSubjects", JSON.stringify(savedActiveStore));
            
            return updated;
          });
        },
...
            <button onClick={generateCaptcha} disabled={isFetching} style={{ background: "#f1f2f6", color: "#2f3640", border: "1px solid #dcdde1", padding: "14px", borderRadius: "12px", flex: 1, fontWeight: "bold", cursor: isFetching ? "not-allowed" : "pointer" }}>⟳ New</button>
            <input type="text" value={captchaInput} onChange={(e) => setCaptchaInput(e.target.value)} disabled={isFetching} placeholder="Type Code" style={{ padding: "14px", borderRadius: "12px", flex: 2, border: "1px solid #dcdde1", fontSize: "16px", outline: "none" }} />
          </div>
          <button 
...
            style={{ 
              padding: "16px", 
              background: isFetching ? "#7f8c8d" : "#2ECC71", 
              color: "#fff", 
              borderRadius: "12px", 
              border: "none", 
              width: "100%", 
...
              boxShadow: isFetching ? "none" : "0 4px 15px rgba(46, 204, 113, 0.3)",
              transition: "all 0.3s ease" 
            }}
          >
            {isFetching ? "⏳ Processing Update..." : "Fetch Live Database Update"}
          </button>
        </div>
      </div>
