...
const AdminFees = () => {
  ...
  // Handle submit → frontend update + backend
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        ...
        setFees(prev => prev.map(f => f.id === form.id ? { ...f, ...form } : f));
      } else {
        // Add new
        const res = await axios.post(`${API_URL}/api/fees`, form);
        // Push new fee to state to show immediately
        const newFee = { ...form, id: Date.now() }; // temporary ID
          ...
          <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
            ...
            <div style={{ ...cardStyle, width: '350px', position: 'sticky', top: '20px' }}>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                ...
                <select value={form.student_id} onChange={handleStudentSelect} required style={{...inputStyle, backgroundColor: !selectedClass?'#f9f9f9':'white'}} disabled={!selectedClass}>
                ...
                {form.id && <button type="button" onClick={()=>{ setForm({...form, id:""}); setSelectedClass(""); }} style={{...primaryBtn, backgroundColor:'#666'}}>Cancel Edit</button>}
              </form>
            </div>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display:'flex', gap:'15px', marginBottom:'20px' }}>
                ...
                <div style={{ ...cardStyle, flex:2, display:'flex', alignItems:'center' }}>
                  ...
                  <input type="text" placeholder="Search by student name..." value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)} style={{...inputStyle,border:'none', fontSize:'16px'}} />
                </div>
              </div>
                  ...
                  <tbody>
                    {filteredFees.map(f=>(
                      <tr key={f.id} style={trStyle}>
                        ...
                        <td style={tdStyle}>
                          <span style={{ ...badge, backgroundColor: f.status==='On Time'?'#e8f5e9':'#fff3e0', color:f.status==='On Time'?'#2e7d32':'#ef6c00'}}>{f.status}</span>
                        </td>
                        <td style={tdStyle}>
